...
class ResNetBasicBlockDO(ResNetResidualBlock):
    ...
    def __init__(self, in_channels, out_channels,dropout=0.2, *args, **kwargs):
        ...
        self.blocks = nn.Sequential(
            ...
            same_conv_block(self.in_channels, self.out_channels, conv_block=conv_block, bias=False,dropout=dropout),
        )

class GoodFellowV3ResNetDO(nn.Module):
    def __init__(self, in_channels=1, n_classes=7, *args, **kwargs):
        super().__init__()
        self.gate = ResNetEncoder(in_channels, block=partial(ResNetBasicBlockDO,dropout=0.0),blocks_sizes=[64], deepths=[2])
